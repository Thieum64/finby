...
export function initOTel(serviceName: string): void {
  ...
  if (sdk) {
    console.warn(`OpenTelemetry already initialized for service: ${serviceName}`);
    return;
  }

  // Don't initialize if ENABLE_OTEL is explicitly set to false
  if (process.env.ENABLE_OTEL === 'false') {
    console.info(`OpenTelemetry disabled via ENABLE_OTEL=false for service: ${serviceName}`);
    return;
  }

  // Don't initialize in test environment
  if (process.env.NODE_ENV === 'test') {
    console.info(`Skipping OpenTelemetry initialization in test environment for service: ${serviceName}`);
    return;
  }
  ...
  try {
    ...
    console.info(`OpenTelemetry initialized successfully for service: ${serviceName}`);
